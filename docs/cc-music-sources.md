...
Freesound carries. Its 100 rows take the whole-page ceiling from 525 to 625.
...
A real `q=jazz` response is now the test fixture, and it falsified **three of the four field
names** the parser had taken from the documentation. Only the `rights` fallback and `id`
survived contact:
...
The date one was the worst: without `year`, the parser fell through to `timestamp_created`,
which is when Europeana ingested the record. A concert given in 2011 was catalogued in 2023,
so every row would have carried the wrong decade while looking perfectly plausible. The
recording date is really in `edmTimespan`, whose language-aware label is tagged `zxx` — the
ISO code for "no linguistic content", because a date is not a word.

Subjects arrive as `dcSubjectLangAware`, an object keyed by language. English is preferred;
the `def` key is skipped whatever happens, because it holds Getty and Europeana vocabulary
URIs rather than words.

One thing to know before pasting a response anywhere: Europeana echoes the caller's `wskey`

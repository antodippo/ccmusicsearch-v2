...
`https://www.loc.gov/audio/?q=…&fo=json` needs **no key at all** — rate-limited only. The
National Jukebox is 5,882 recordings from 1900–1922, public domain under the Music
Modernization Act: a genuinely distinct catalogue nothing else here touches. No licence URL in
the payload, so `PUBLIC_DOMAIN` would be hard-coded — which, unlike Icons8's `UNKNOWN`, is a
*true* statement about a known-public-domain collection. Paging is capped at 100,000 results.
Worth doing next.
...
catalogue fit found anywhere. But it runs Ampache, whose API key can only be generated by a
server administrator for a specific user; there is no anonymous access. It also spans 44
licences, some non-CC (the Free Art License would land as `UNKNOWN`). Worth an email to them —
this is a relationship problem, not a technical one.
